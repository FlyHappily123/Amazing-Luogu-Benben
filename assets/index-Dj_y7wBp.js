...
class d {
    ...
    expandSidebar() {
        this.sidebar && (this.sidebar.classList.add("expanded"),
        this.isMobile && (this.sidebarOverlay.classList.add("active"),
        document.body.style.overflow = "hidden"),
        this.isManuallyCollapsed = !1)
    }
    collapseSidebar() {
        this.sidebar && (this.sidebar.classList.remove("expanded"),
        this.isMobile && (this.sidebarOverlay.classList.remove("active"),
        document.body.style.overflow = ""))
    }
    manuallyCollapseSidebar() {
        !this.isMobile && this.sidebar.classList.contains("expanded") && (this.collapseSidebar(),
        this.isManuallyCollapsed = !0,
        setTimeout( () => {
            this.isManuallyCollapsed = !1
        }
        , 5e3))
    ...
    toggleSidebar() {
        this.isMobile ? this.sidebar.classList.contains("expanded") ? this.collapseSidebar() : this.expandSidebar() : this.sidebar.classList.contains("expanded") ? this.manuallyCollapseSidebar() : this.expandSidebar()
    }
    handleTouchStart(e) {
        this.isMobile || (this.touchStartX = e.touches[0].clientX,
        this.isTouching = !0,
        e.target.closest(".sidebar") && this.expandSidebar())
    }
    handleTouchEnd(e) {
        this.isMobile || (this.touchEndX = e.changedTouches[0].clientX,
        this.isTouching = !1,
        e.target.closest(".sidebar") && Math.abs(this.touchEndX - this.touchStartX) < 10 && setTimeout( () => {
            !this.sidebar.matches(":hover") && !this.isManuallyCollapsed && this.collapseSidebar()
        }
        , 300))
    }
    handleTouchLeave() {
        this.isMobile || this.isTouching && (this.collapseSidebar(),
        this.isTouching = !1)
    }
    handleTouchCancel() {
        this.isMobile || (this.isTouching = !1,
        this.collapseSidebar())
    }
    handleMouseEnter() {
        this.isMobile || (this.leaveTimeout && (clearTimeout(this.leaveTimeout),
        this.leaveTimeout = null),
        this.isManuallyCollapsed || (this.hoverTimeout && clearTimeout(this.hoverTimeout),
        this.hoverTimeout = setTimeout( () => {
            this.isManuallyCollapsed || this.expandSidebar()
        }
        , 100)))
    }
    handleMouseLeave(e) {
        ...
        this.sidebar.contains(t) || this.isManuallyCollapsed || (this.leaveTimeout && clearTimeout(this.leaveTimeout),
    ...
    handleResize() {
        ...
        this.isMobile = this.checkIfMobile(),
        e && !this.isMobile ? (this.collapseSidebar(),
        this.sidebarOverlay.classList.remove("active"),
        document.body.style.overflow = "",
        this.isManuallyCollapsed = !1,
        this.sidebar.style.transform = "") : !e && this.isMobile && (this.collapseSidebar(),
        this.isManuallyCollapsed = !1)
    }
    handleSidebarClick(e) {
        this.isMobile || e.target.closest(".nav-item") && (e.preventDefault(),
        this.expandSidebar(),
        this.isManuallyCollapsed = !1)
    }
    initEventListeners() {
        ...
        this.sidebarOverlay.addEventListener("click", t => {
            this.isMobile && (t.stopPropagation(),
            this.collapseSidebar())
        }
        ),
        this.sidebar.addEventListener("touchstart", t => this.handleTouchStart(t)),
        this.sidebar.addEventListener("touchend", t => this.handleTouchEnd(t)),
        ...
        this.sidebar.addEventListener("mouseleave", t => this.handleMouseLeave(t)),
        this.sidebar.addEventListener("click", t => this.handleSidebarClick(t)),
        document.querySelectorAll(".nav-item").forEach(t => {
            t.addEventListener("click", i => {
                this.isMobile ? this.collapseSidebar() : (i.preventDefault(),
                this.expandSidebar(),
                this.isManuallyCollapsed = !1)
            }
            )
        ...
        document.addEventListener("click", t => {
            this.isMobile && this.sidebar.classList.contains("expanded") && !this.sidebar.contains(t.target) && !this.menuToggle.contains(t.target) && !this.sidebarOverlay.contains(t.target) && this.collapseSidebar()
        }
        ),
        document.addEventListener("keydown", t => {
            t.key === "Escape" && (this.isMobile && this.sidebar.classList.contains("expanded") ? this.collapseSidebar() : this.isMobile || this.manuallyCollapseSidebar())
        }
        )
    }
    init() {
        this.sidebar && (this.isMobile ? this.collapseSidebar() : (this.collapseSidebar(),
        this.isManuallyCollapsed = !1),
...
class h {
    ...
    setupUsersPage() {
        const e = document.getElementById("userSearch");
        e && e.addEventListener("input", s => {
            this.usersSearch = s.target.value.toLowerCase(),
            this.usersPage = 1,
            this.renderUsersContent()
        }
        );
        const t = document.querySelectorAll(".view-toggle");
        t.forEach(s => {
            s.addEventListener("click", () => {
                t.forEach(a => a.classList.remove("active")),
                s.classList.add("active"),
                this.usersView = s.getAttribute("data-view"),
                this.usersPage = 1,
                this.renderUsersContent()
            }
            )
        ...
        const i = document.getElementById("refreshUsers");
    ...
    async fetchUsersData() {
        try {
            const e = await fetch(this.SUPABASE_URL + "/rest/v1/user_status?select=uid,last_seen", {
                headers: {
            ...
            throw console.error("获取用户数据失败:", e),
            e
        }
    }
    async loadUsersData() {
    ...
    `);
        try {
            const t = await this.fetchUsersData();
            this.processUsersData(t),
            this.renderUsersContent()
        } catch {
    ...
    async refreshUsersData() {
        const e = document.getElementById("refreshUsers");
    ...
    processUsersData(e) {
        const t = Date.now()
          , i = 300 * 1e3
          , s = new Set
          , a = e.filter(n => s.has(n.uid) ? !1 : (s.add(n.uid),
        !0)).map(n => {
            const r = new Date(n.last_seen).getTime();
            return {
                uid: String(n.uid),
                lastSeen: r,
                isOnline: t - r <= i,
                lastSeenFormatted: this.formatLastSeen(r)
            }
        }
        ).sort( (n, r) => n.uid - r.uid);
        this.usersData = a
    }
    formatLastSeen(e) {
        const i = Date.now() - e
          ...
          , a = Math.floor(s / 60)
          , n = Math.floor(a / 24);
        return i <= 300 * 1e3 ? "刚刚在线" : s < 60 ? `${s}分钟前` : a < 24 ? `${a}小时前` : n < 30 ? `${n}天前` : new Date(e).toLocaleDateString("zh-CN")
    }
    renderUsersContent() {
        const e = this.usersData.length
          , t = this.usersData.filter(r => r.isOnline).length;
        document.getElementById("total-users").textContent = e,
        document.getElementById("online-users").textContent = t,
        document.getElementById("last-updated").textContent = new Date().toLocaleTimeString("zh-CN", {
            hour: "2-digit",
            minute: "2-digit"
        });
        let i = this.usersData.filter(r => !(this.usersSearch && !r.uid.includes(this.usersSearch) || this.usersView === "online" && !r.isOnline || this.usersView === "offline" && r.isOnline));
        const s = Math.ceil(i.length / this.usersPerPage)
          , a = (this.usersPage - 1) * this.usersPerPage
          , n = i.slice(a, a + this.usersPerPage);
        this.renderUserList(n),
        this.renderPaginationInfo(i.length),
        this.renderPaginationControls(s)
    }
    renderUserList(e) {
  ...
  `).join("")
    ...
    renderPaginationInfo(e) {
        const t = (this.usersPage - 1) * this.usersPerPage + 1
          , i = Math.min(this.usersPage * this.usersPerPage, e);
        document.getElementById("paginationInfo").innerHTML = `
    显示第 ${t}–${i} 个用户，共 ${e} 个用户
  `
    }
    renderPaginationControls(e) {
        const t = document.getElementById("paginationControls");
        if (e <= 1) {
            t.innerHTML = "";
            return
        }
  ...
  `;
        for (let s = 1; s <= e; s++)
            s === 1 || s === e || s >= this.usersPage - 2 && s <= this.usersPage + 2 ? i += `
        <button class="pagination-btn ${s === this.usersPage ? "active" : ""}" 
                onclick="window.pageManager.changeUsersPage(${s})">
          ${s}
        </button>
      ` : (s === this.usersPage - 3 || s === this.usersPage + 3) && (i += '<span class="pagination-btn" style="border: none; cursor: default;">...</span>');
        i += `
    <button class="pagination-btn" onclick="window.pageManager.changeUsersPage(${this.usersPage + 1})" 
  ...
  `,
    ...
    async deleteUsersByUID(e) {
        const t = e.join(",")
          , i = `${this.SUPABASE_URL}/rest/v1/user_status?uid=in.(${t})`;
        try {
            ...
            return s.ok ? (console.log("删除成功:", e),
            !0) : (console.error("删除失败:", s.status, await s.text()),
            !1)
        } catch (s) {
            return console.error("删除用户失败:", s),
            !1
        }
    }
    async preloadData() {
        this.websiteData.length === 0 && await this.loadData("website"),
        this.scriptData.length === 0 && await this.loadData("script")
    }
    async fetchData(e) {
        const t = e === "website" ? "websites" : "scripts"
          , i = e === "website" ? this.websitesJsonPath : this.scriptsJsonPath;
        if (this.dataCache[t] && this.dataCache.timestamp && Date.now() - this.dataCache.timestamp < this.dataCache.cacheDuration)
            return console.log(`使用缓存的${e}数据`),
            this.dataCache[t];
        try {
            this.showLoading(e);
            const s = await fetch(`${i}?t=${Date.now()}`, {
                headers: {
            ...
            const a = await s.json()
              , n = e === "website" ? a.websites || a : a.scripts || a;
            return !n || !Array.isArray(n) ? (console.warn("数据格式不符合预期，尝试解析原始数据"),
            a) : (this.dataCache[t] = n,
            this.dataCache.timestamp = Date.now(),
            console.log(`成功加载${e}数据`, n),
            n)
        } catch (s) {
            console.error(`加载${e}数据失败:`, s);
            const a = this.getDefaultData(e);
            return this.dataCache[t] = a,
            this.dataCache.timestamp = Date.now(),
            a
        }
    }
    ...
    showLoading(e) {
        const t = document.getElementById(`${e}-grid`);
        t && (t.innerHTML = `
        <div class="loading-container">
          <div class="loading-spinner"></div>
    ...
    showError(e, t) {
        const i = document.getElementById(`${e}-grid`);
        i && (i.innerHTML = `
    ...
    async loadData(e) {
        try {
            ...
            e === "website" ? (this.websiteData = t,
            this.renderWebsiteContent()) : (this.scriptData = t,
            this.renderScriptContent())
        } catch (t) {
            this.showError(e, t.message)
        }
    }
    async refreshAllData() {
        ...
        try {
            ...
            await Promise.all([this.loadData("website"), this.loadData("script")]),
            this.showNotification("数据刷新成功！", "success")
        } catch (t) {
            console.error("刷新数据失败:", t),
    ...
    `,
        Object.assign(i.style, {
            ...
            top: "80px",
            right: "20px",
            background: t === "success" ? "var(--success-color)" : "var(--danger-color)",
            color: "white",
            padding: "12px 20px",
            borderRadius: "8px",
            display: "flex",
    ...
    setupNavigation() {
        document.querySelectorAll(".nav-item").forEach(t => {
            t.addEventListener("click", i => {
                i.preventDefault();
                const s = t.getAttribute("data-page");
                s !== "none" && this.navigateTo(s)
            }
            )
        }
        )
  ...
  `;
    ...
    updateNavActive(e) {
        document.querySelectorAll(".nav-item").forEach(i => {
            ...
            i.getAttribute("data-page") === e && i.classList.add("active")
        }
        )
    }
    loadInitialPage() {
    ...
    setupExplorePage() {
        const e = document.getElementById("searchInput")
          , t = document.getElementById("searchBtn")
          , i = () => {
            this.searchKeyword = e.value.toLowerCase(),
        ...
        const s = document.querySelectorAll(".filter-tab");
        s.forEach(o => {
            o.addEventListener("click", () => {
                s.forEach(l => l.classList.remove("active")),
                o.classList.add("active"),
                this.activeFilter = o.getAttribute("data-filter"),
                this.renderExploreContent()
            }
            )
        }
        );
        const a = document.querySelectorAll("#website-tags .tag-filter-item");
        a.forEach(o => {
            o.addEventListener("click", () => {
                a.forEach(l => l.classList.remove("active")),
                o.classList.add("active"),
                this.activeWebsiteTag = o.getAttribute("data-tag"),
                this.renderWebsiteContent()
            }
            )
        }
        );
        const n = document.querySelectorAll("#script-tags .tag-filter-item");
        n.forEach(o => {
            o.addEventListener("click", () => {
                n.forEach(l => l.classList.remove("active")),
                o.classList.add("active"),
                this.activeScriptTag = o.getAttribute("data-tag"),
                this.renderScriptContent()
            }
            )
        }
    ...
    renderWebsiteContent() {
        const e = document.getElementById("website-grid")
          , t = document.getElementById("website-count");
        if (!this.websiteData || this.websiteData.length === 0) {
            this.showEmptyState("website"),
            t && (t.textContent = "0");
            return
        }
        let i = this.websiteData.filter(s => {
            if (this.activeFilter === "script" || this.activeWebsiteTag !== "all" && !s.tags.includes(this.activeWebsiteTag))
                return !1;
            if (this.searchKeyword) {
                const a = s.name.toLowerCase().includes(this.searchKeyword)
                  , n = s.description.toLowerCase().includes(this.searchKeyword)
                  , r = s.tags.some(o => o.toLowerCase().includes(this.searchKeyword));
                if (!a && !n && !r)
                    return !1
            }
            return !0
    ...
    renderScriptContent() {
        const e = document.getElementById("script-grid")
          , t = document.getElementById("script-count");
        if (!this.scriptData || this.scriptData.length === 0) {
            this.showEmptyState("script"),
            t && (t.textContent = "0");
            return
        }
        let i = this.scriptData.filter(s => {
            if (this.activeFilter === "website" || this.activeScriptTag !== "all" && !s.tags.includes(this.activeScriptTag))
                return !1;
            if (this.searchKeyword) {
                const a = s.name.toLowerCase().includes(this.searchKeyword)
                  , n = s.description.toLowerCase().includes(this.searchKeyword)
